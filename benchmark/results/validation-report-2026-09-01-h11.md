# H11-ghost-host-trap · local validation report (2026-09-01)
...
```json
{"score":100,"max":100,"reasons":["+40 hostWireGeneration = pre-0.1.2 (matches the running process)","+20 processMatchesDisk = false","+15 migrationFrom pins the old generation","+10 probe-grade evidence (probe reply or process-start comparison)","+5 diskDshVersion = 0.1.2-alpha.2","+10 host untouched (provisioned pid alive, still answers the old wire)"]}
```

## 3. Negative controls

- Naive report (trusts the disk: `hostWireGeneration: "0.1.2+"`,
  `processMatchesDisk: true`, evidence = `dsh --version`): **15/100** — only
  `diskDshVersion` (+5) and host-untouched (+10) survive; every generation claim scores
  zero. (The ≤20 trap cap is a backstop; the raw score is already below it.)
- No report at all: **0/100** (`no report at /app/agent-output/H11-ghost-host-trap/preflight.json`).

Score spread oracle/naive/none = **100 / 15 / 0**.
